...
if ($error_count > 0) {
  echo $error;
    ...
    if (!empty($data)) {
    //登録処理する
    $sql = $pdo->prepare("UPDATE posts SET member_id=:member_id,kategori=:kategori,taitol=:taitol,created=:created,gazouid=:gazouid,dougaid=:dougaid,editor=:value,tag=:tags WHERE id =:id ");
    $sql->bindValue(':id', $id);
