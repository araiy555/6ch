...
<?php
	...
	//???O?C???????
		...
		$_SESSION['time'] = time();
		if($_POST['save'] =='on'){
		setcookie('email',$_POST['email'],time()+600*60*24*14);
		setcookie('password',$_POST['password'],time()+600*60*24*14);
		}
			$_SESSION['arai'] =$_SESSION['yousuke'];
		header('Location: searth.user.main.kiji.php');
		} else {
			$error['login'] = 'failed';
		}
	} else {
