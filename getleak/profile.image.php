...
$books_num = count($gazou2);
$max_page = ceil($books_num / MAX);

if(!isset($_GET['gazou_id'])){
    $now = 1;
}else{
    $now = $_GET['gazou_id'];
}
...
$disp_data1 = array_slice($gazou2, $start_no, MAX, true);
...
<div class="contents">
    ...
    if($now < $max_page){ // リンクをつけるかの判定
              echo '<div class="pager"><a class="pager__next" href=\'/profile.image.php?gazou_id='.($now + 1).'\')>次へ</a></div>'. '　';
    } else {
}
    ?>
</div>
