...
$id = $member['id'];
 ...
 $sql = sprintf("SELECT * FROM  ibent WHERE member_id = '%s' ORDER BY created DESC",
...
<form action="searth.php" name="search1" method="post">
	<dl class="search1" >
		<dt><input type="text" name="search" value="" placeholder="Earthの検索またはチャンネル" autocomplete="off"/></dt>
...
$lod = $_REQUEST['id'];
 $sql = sprintf(" SELECT  member_id FROM ibent WHERE id=%d;",
...
$id = $postsss['member_id'];
 ...
 $sql = sprintf("SELECT m.seibetu,m.id, m.simei, m.name,m.tosi,m.kokuseki, p.id, p.member_id, p.basyo,p.url, p.taitol,p.created,p.url, p.gazouid,p.dougaid FROM members m ,ibent p  WHERE p.member_id = '%s' AND p.id = %d ORDER BY created DESC",
  mysql_real_escape_string($id),
  mysql_real_escape_string($lod)
);
...
$id = $postsss['member_id'];
 $sql = sprintf("SELECT m.*, p.* FROM members m, ibent p WHERE m.id=%d",
    mysql_real_escape_string($id)
);
