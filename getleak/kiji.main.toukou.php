...
$id = $postsss['member_id'];
 ...
 $sql = sprintf("SELECT m.seibetu,m.id, m.simei, m.name,m.tosi,m.kokuseki, p.id, p.member_id, p.kategori,p.kiji, p.taitol,p.created, p.gazouid,p.dougaid,p.editor FROM members m ,posts p  WHERE p.member_id = '%s' AND p.id = %d ORDER BY created DESC",
  mysql_real_escape_string($id),
  mysql_real_escape_string($lod)
);
...
<?php
	...
	?>
		...
		<?php echo $postcc['kiji']; ?>
	<?php $value = htmlspecialchars_decode($postcc['editor']);
        echo $value;?>
