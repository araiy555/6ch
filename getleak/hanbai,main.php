...
<a href="kiji.main.php" class="modal-close">&times;</a>
	...
	?>	
		...
		<a href="main.php"><?php echo $member['simei']; ?>
...
$id = $postsss['member_id'];
 ...
 $sql = sprintf("SELECT m.seibetu,m.id, m.simei, m.name,m.tosi,m.kokuseki, p.id, p.member_id, p.kategori,p.url, p.nedan, p.taitol,p.created, p.gazouid,p.dougaid FROM members m ,hanbaiposts p  WHERE p.member_id = '%s' AND p.id = %d ORDER BY created DESC",
  mysql_real_escape_string($id),
  mysql_real_escape_string($lod)
);
