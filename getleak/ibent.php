...
<li><a href="jinbutu.php?id=<?php echo $name; ?>">チャンネル</a></li>
...
<?php
    if (isset($name)) {
        ...
        $url = "http://eventon.jp/api/events.xml?&keyword=".$keyword."&limit=20";
        $xml = simplexml_load_file($url);
		$data = get_object_vars($xml);


		  foreach($data['events']->event as $xml){

            		echo '<li>'. $xml->title. '</li>';
		

			echo '<a href="'.$xml->event_url.'">'. $xml->event_url.'</a>';
		}
